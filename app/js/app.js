/* ===========================================================
   TH · SỔ QUẢN LÝ TÀI CHÍNH CÁ NHÂN — app.js
...
// 6 LỌ JARS mặc định (chuẩn T. Harv Eker). Người dùng có thể tự chỉnh % & tên trong app.
const DEFAULT_JARS = [
  {id:'nec', icon:'🏠', name:'Thiết Yếu (NEC)',         pct:55, desc:'Ăn ở, đi lại, hóa đơn — nhu cầu sống cơ bản'},
  {id:'ffa', icon:'💰', name:'Tự Do Tài Chính (FFA)',   pct:10, desc:'Đầu tư sinh lời, KHÔNG tiêu — "con ngỗng vàng"'},
  {id:'ltss',icon:'🎯', name:'Tiết Kiệm Dài Hạn (LTSS)',pct:10, desc:'Mua sắm lớn, dự phòng, mục tiêu dài hạn'},
  {id:'edu', icon:'📚', name:'Giáo Dục (EDU)',          pct:10, desc:'Học hành, sách vở, khóa học phát triển bản thân'},
  {id:'play',icon:'🎉', name:'Hưởng Thụ (PLAY)',        pct:10, desc:'Tự thưởng, giải trí — tiêu hết mỗi tháng'},
  {id:'give',icon:'🎁', name:'Cho Đi (GIVE)',           pct:5,  desc:'Từ thiện, quà tặng, giúp đỡ người khác'}
];
// Số ngày kể từ lần sao lưu gần nhất sẽ nhắc người dùng xuất file dự phòng
...
function blank(){ return { transactions:[], wallets:[], budgets:{}, recurring:[], balances:{}, goals:{}, reflections:{}, commits:{}, jars:null, meta:{} }; }
function load(){
  let d;
  ...
  if(!Array.isArray(d.jars) || !d.jars.length){ d.jars = DEFAULT_JARS.map(j=>({...j})); }
...
/* ===========================================================
   6 LỌ JARS
   =========================================================== */
function jarsSumPct(){ return state.jars.reduce((s,j)=>s+(Number(j.pct)||0),0); }
// Phân bổ thu nhập theo từng lọ → [{...jar, amount}]
function jarsSplit(income){ return state.jars.map(j=>({...j, amount: income*(Number(j.pct)||0)/100})); }
function resetJars(){ state.jars = DEFAULT_JARS.map(j=>({...j})); save(); }
...
incAmountInput.addEventListener('input',()=>{
  ...
  box.innerHTML = 'Chia 6 lọ: ' + jarsSplit(v).map(j=>
    `<b title="${esc(j.name)}">${j.icon} ${fmtShort(j.amount)}</b>`).join(' · ');
});
...
function renderDashboard(){
  ...
  document.getElementById('fundsBox').innerHTML=
    jarsSplit(mt.income).map(j=>fundRow(`${j.icon} ${j.name}`, j.pct+'%', j.amount)).join('');
...
/* ===========================================================
   JARS VIEW (6 lọ — chỉnh tên & %)
   =========================================================== */
function renderJars(){
  const mt=monthTotals(curMonth);
  const editor=document.getElementById('jarsEditor');
  editor.innerHTML=state.jars.map((j,i)=>`
    <div class="jar-edit">
      <div class="jar-ic">${j.icon}</div>
      <div class="jar-fields">
        <input class="jar-name" data-ji="${i}" value="${esc(j.name)}" maxlength="40" />
        <p class="jar-desc">${esc(j.desc||'')}</p>
      </div>
      <div class="jar-pct-wrap"><input type="number" class="jar-pct" data-ji="${i}" min="0" max="100" step="1" value="${j.pct}" /><span>%</span></div>
      <div class="jar-amt" data-ji="${i}">${fmt(mt.income*(Number(j.pct)||0)/100)}</div>
    </div>`).join('');
  document.getElementById('jarsIncomeNote').textContent = mt.income
    ? `Phân bổ theo thu nhập tháng ${curMonth.split('-').reverse().join('/')}: ${fmt(mt.income)}`
    : 'Chưa có thu nhập trong tháng này — nhập một khoản Thu để xem số tiền mỗi lọ.';
  updateJarsAmounts();
  editor.querySelectorAll('.jar-name').forEach(inp=>inp.addEventListener('change',()=>{
    const v=inp.value.trim(); if(v) state.jars[inp.dataset.ji].name=v; save();
  }));
  editor.querySelectorAll('.jar-pct').forEach(inp=>inp.addEventListener('input',()=>{
    const v=Math.max(0,Math.min(100,Math.round(Number(inp.value)||0)));
    state.jars[inp.dataset.ji].pct=v; save(); updateJarsAmounts();
  }));
}
// Cập nhật số tiền mỗi lọ + ghi chú tổng % mà không dựng lại ô nhập (giữ con trỏ)
function updateJarsAmounts(){
  const mt=monthTotals(curMonth), sum=jarsSumPct();
  document.querySelectorAll('#jarsEditor .jar-amt').forEach(el=>{
    const j=state.jars[el.dataset.ji]; el.textContent=fmt(mt.income*(Number(j.pct)||0)/100);
  });
  const note=document.getElementById('jarsSumNote');
  if(note){
    if(Math.round(sum)===100){ note.className='jars-sum ok'; note.innerHTML='✅ Tổng phân bổ: <b>100%</b>'; }
    else { note.className='jars-sum warn'; note.innerHTML=`⚠ Tổng phân bổ: <b>${sum}%</b> — nên bằng 100% (đang ${sum>100?'thừa '+(sum-100):'thiếu '+(100-sum)}%).`; }
  }
}
...
document.getElementById('exportBtn').addEventListener('click',()=>{
  ...
  a.download='kt-so-tai-chinh-'+todayISO()+'.json'; a.click();
  state.meta.lastBackup=todayISO(); save(); toast('Đã xuất dữ liệu — đã lưu mốc sao lưu');
  const bn=document.getElementById('backupBanner'); if(bn) bn.hidden=true;
});
...
/* ===========================================================
   ĐỒNG HÀNH KHÓA HỌC + NHẮC SAO LƯU
   =========================================================== */
function daysBetween(isoA,isoB){ return Math.round((Date.parse(isoB)-Date.parse(isoA))/86400000); }
// Banner nhắc xuất file dự phòng (localStorage có thể mất khi xóa cache/đổi máy)
function checkBackupReminder(){
  const bn=document.getElementById('backupBanner'); if(!bn) return;
  if(!state.transactions.length){ bn.hidden=true; return; }
  const last=state.meta.lastBackup;
  const stale = !last || daysBetween(last,todayISO())>=BACKUP_REMIND_DAYS;
  bn.hidden=!stale;
}
/* ---------- BOOT ---------- */
fillCatSelects();
...
applyRecurring();
checkBackupReminder();
document.getElementById('resetJarsBtn').addEventListener('click',()=>{
  if(!confirm('Khôi phục 6 lọ về mặc định JARS (55/10/10/10/10/5)? Tên & % bạn đã chỉnh sẽ bị thay.')) return;
  resetJars(); renderJars(); toast('Đã khôi phục 6 lọ mặc định');
});
document.getElementById('backupDismiss').addEventListener('click',()=>{ document.getElementById('backupBanner').hidden=true; });
document.getElementById('backupNowBtn').addEventListener('click',()=>document.getElementById('exportBtn').click());
